...
int main(int argc, char** argv)
{
   ...
   if(isPromptD0){
      ...
      if(prefix.size())
         outName = TString::Format("%s/fout_%s_%s_ntrk_pT%.1f-%.1f_y%.1f-%.1f.root", prefix.c_str(), 
               datalist.c_str(), training.c_str(), cuts.pTMin, cuts.pTMax, cuts.yMin, cuts.yMax);
      else
         outName = TString::Format("fout_%s_%s_ntrk_pT%.1f-%.1f_y%.1f-%.1f.root", datalist.c_str(), training.c_str(),
