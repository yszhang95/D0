...
bool passD0MVA(const int&, Event*, const int&, const bool&, const float&);
...
int main(int argc, char** argv)
{
   ...
   TH1D* hMass_D0[nMass][nPt][nDca];

   TH2D* hNtrkofflineVsNtrkgood;

   TH3D* hDcaVsMassAndMva[nPt];

   TH1D* hMult_raw_D0[nMass][nPt][nDca];
   TH1D* hMult_eff_D0[nMass][nPt][nDca];
   TH2D* hSignal_D0[nMass][nPt][nDca];
   TH2D* hBackground_D0[nMass][nPt][nDca];

   TH1D* hDca[nMass][nPt][nDca];
   ...
   // declare vectors
   vector<TVector3> pVect_trg_d0[nMass][nPt];
   vector<double> effVect_trg_d0[nMass][nPt];

   vector<TVector3> pVect_dau1_d0[nMass][nPt];

   vector<TVector3> pVect_dau2_d0[nMass][nPt];

   vector<int>      indexVect_d0[nMass][nPt];
   ...
   for(long int ientry=0; ientry<nentries; ientry++){
      ...
      unsigned int nMult_trg_raw_d0[nMass][nPt][nDca]; // Ntrig for mass & pt bins
      double nMult_trg_eff_d0[nMass][nPt][nDca]; // eff corrected Ntrig for mass & pt bins
   ...
   if(isPromptD0){
      ...
      if(prefix.size())
         outName = TString::Format("%s/fout_%s_%s_HM%03d-%03d_pT%.1f-%.1f_y%.1f-%.1f_%.2f.root", 
               prefix.c_str(), datalist.c_str(), training.c_str(), ntrkBounds.first, ntrkBounds.second, 
               cuts.pTMin, cuts.pTMax, cuts.yMin, cuts.yMax, mvacut);
      else
         outName = TString::Format("fout_%s_%s_HM%03d-%03d_pT%.1f-%.1f_y%.1f-%.1f_%.2f.root", 
...
inline bool passD0Selections(const int& trigger, Event* event, const int& icand, const bool& isPromptD0)
{
   ...
   if(!passD0MVA(trigger, event, icand, isPromptD0, mvacut)) return false;
   return true;
}
...
bool passD0MVA(const int& trigger, Event* event, const int& icand, 
      const bool& isPrompt, const float& mvacut)
{
   if(isPrompt){
