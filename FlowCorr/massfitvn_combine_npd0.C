...
const int nDca= 3;
const double dcaBin[nDca+1] = {0., 0.009, 0.014, 10000.};
...
void massfitvn_combine_npd0_eachDca(const float yMax, const char* data_file, TFile& ofile, const int idca)
{
   gStyle->SetOptStat(0);
    ...
    TF1* fmasssig[ana::nPt];
    TF1* fmassswap[ana::nPt];
    TF1* fmassbkg[ana::nPt];
    TF1* fmasstotal[ana::nPt];
    TF1* fvn[ana::nPt];
    
    double pt[ana::nPt];
    double KET_ncq[ana::nPt];
    double v2[ana::nPt];
    double v2e[ana::nPt];
    double v2_bkg[ana::nPt];
    double v2_ncq[ana::nPt];
    double v2e_ncq[ana::nPt];
    //double ptbin[14] = {1.5,2.4,3,3.5,4.2,5,6,7,8,10,11.5,15,20,30};
    double ptbin[] = {2., 4., 6., 8.};
    double a[ana::nPt];
    double b[ana::nPt];
    double sigfrac[ana::nPt];
    
    TCanvas* c[ana::nPt];
    for(int i=0;i<ana::nPt;i++)
    {
    ...
    TCanvas* c2 = new TCanvas(Form("c2_dca%d", idca),"c2",500,500);
    ...
    for(int i=0;i<ana::nPt;i++)
    {
        TFile* file0 = TFile::Open(Form("MC/hMC_mass_npd0ana1_y%.1f.root", yMax));
        TFile* file1 = TFile::Open(data_file);
        TH1D* h_mc_match_signal = (TH1D*)file0->Get(Form("hMC_mass_pt%d_dca%d",i, idca));
        TH1D* h_mc_match_all = (TH1D*)file0->Get(Form("hMC_mass_all_pt%d_dca%d",i, idca));
        ...
        if(idca!=nDca-1)tex->DrawLatex(0.22,0.64,Form("%.3f<DCA<%.3fmm", dcaBin[idca], dcaBin[idca+1]));
        if(idca==nDca-1)tex->DrawLatex(0.22,0.64,Form("DCA>%.3fmm", dcaBin[idca]));
        ...
        tex->DrawLatex(0.22,0.86,"185 #leq N_{trk}^{offline} < 250");
        tex->DrawLatex(0.22,0.80,Form("%.1f < p_{T} < %.1f GeV/c", ana::ptbin[i], ana::ptbin[i+1]));
        tex->DrawLatex(0.22,0.74,Form("|y| < %.1f", yMax));
...
//        leg1->AddEntry(f3,"Combinatorial","l");
    ...
    ofile.cd();
    const double x_e[] = {1, 1, 1};
    const double pt_bin[] = {3., 5., 7.};
    TGraphErrors* v2plot = new TGraphErrors(3, pt_bin, v2, x_e,v2e);
    auto h = v2plot->GetHistogram();
    h->SetTitle(";p_{T} (GeV);signal v_{2}");
   ...
   texCMS->DrawLatex(0.62,0.97, "#scale[0.8]{pPb #sqrt{s_{NN}} = 8.16 TeV}");
   tex->DrawLatex(0.22,0.86,"185 #leq N_{trk}^{offline} < 250");
   tex->DrawLatex(0.22,0.78,Form("|y| < %.1f", yMax));
   if(idca!=nDca-1)tex->DrawLatex(0.22,0.64,Form("%.3f<DCA<%.3fmm", dcaBin[idca], dcaBin[idca+1]));
   if(idca==nDca-1)tex->DrawLatex(0.22,0.64,Form("DCA>%.3fmm", dcaBin[idca]));
    c2->Print(Form("plots/dcaFull/v2/signalv2_dca%d.png", idca));
    delete h;
    /*
...
void massfitvn_combine_npd0()
{
   const float yMax=2.0;
   TFile ofile(Form("data/v2vspt_d0_HM185_250_y%.1f_dcafull.root", yMax),"RECREATE");
   const std::string data_file= "data/corr2D_npd0ana1_dcaFull_y2.0.root_v2.root";
   for(int idca=0; idca<nDca; idca++){
   //for(int idca=0; idca<1; idca++){
      massfitvn_combine_npd0_eachDca(yMax, data_file.c_str(), ofile, idca);
   }
}
