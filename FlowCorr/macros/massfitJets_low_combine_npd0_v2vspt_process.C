...
void massfitJets_low_combine_npd0_v2vspt_process(
      const char* input_mc = "", 
      ...
      )
{
    ...
    for(int i=0;i<nPt;i++)
    {
        ...
        TGraphErrors* vn_data = (TGraphErrors*)file1->Get(Form("Jets_lowZYAMvsMass_pt%d_dca%d",i,idca));
        ...
        vn_data->Fit(fvn_combinemassvnfit, "QRE", "", fit_range_low, fit_range_high);
        vn_data->Fit(fvn_combinemassvnfit, "QRE", "", fit_range_low, fit_range_high);
        vn_data->Fit(fvn_combinemassvnfit, "QRE", "", fit_range_low, fit_range_high);
        vn_data->Fit(fvn_combinemassvnfit, "REV", "", fit_range_low, fit_range_high);
