      ...
      )
{
   ...
   for(int i=0; i<2; i++){
      ...
      ltx->DrawLatexNDC(0.24, 0.7, Form("%.1f #leq p_{T} < %.1fGeV", ptbin[i], ptbin[i+1]));
      ltx->DrawLatexNDC(0.32, 0.63, "|y|<1");
      ...
      if(outputs == "Jets_low") ltx->DrawLatexNDC(0.22, 0.78, "N_{trk}^{offline} < 35");
      //if(outputs == "Jets") ltx->DrawLatexNDC(0.3, 0.2, " 185#leq N_{trk}^{offline} < 250");
      if(outputs == "Jets") ltx->DrawLatexNDC(0.22, 0.78, " 185#leq N_{trk}^{offline} < 250");
      //if(outputs == "V2_low") ltx->DrawLatexNDC(0.3, 0.2, "N_{trk}^{offline} < 35");
      if(outputs == "V2_low") ltx->DrawLatexNDC(0.22, 0.78, "N_{trk}^{offline} < 35");
      //if(outputs == "V2") ltx->DrawLatexNDC(0.3, 0.2, " 185#leq N_{trk}^{offline} < 250");
      if(outputs == "V2") ltx->DrawLatexNDC(0.22, 0.78, " 185#leq N_{trk}^{offline} < 250");
      ...
      if(outputs == "Jets_low") h->GetYaxis()->SetTitle("Y_{jets}^{signal}");
      if(outputs == "Jets") h->GetYaxis()->SetTitle("Y_{jets}^{signal}");
      if(outputs == "V2_low") h->GetYaxis()->SetTitle("V_{2}^{signal}");
      if(outputs == "V2") h->GetYaxis()->SetTitle("V_{2}^{signal}");
      if(outputs == "V2" || outputs == "V2_low") h->GetYaxis()->SetTitleOffset(1.65);
