...
void massfitVn_low_combine_npd0_v2vspt_process_new(
      const char* input_mc = "", 
      ...
      )
{
    ...
    for(int i=0;i<nPt;i++)
    {
        ...
        TGraphErrors* vn_data = (TGraphErrors*)file1->Get(Form("V2_lowvsMass_pt%d_dca%d",i,idca));
        ...
        vn_data->Fit(fvn_combinemassvnfit, "Q R E", "", fit_range_low, fit_range_high);
        vn_data->Fit(fvn_combinemassvnfit, "Q R E", "", fit_range_low, fit_range_high);
        vn_data->Fit(fvn_combinemassvnfit, "R E V", "", fit_range_low, fit_range_high);
