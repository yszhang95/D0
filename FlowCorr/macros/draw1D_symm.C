...
void draw_proj1D_symm(string dataset = "", string input = "", string output = "",
      ...
      unordered_map<string, TGraphErrors*>* graph_map = nullptr)
{
   ...
   for(int imass=0; imass<ana::nMass; imass++){
      ...
      int binlw =hsr[imass]->GetXaxis()->FindBin(0.);
   ...
   for(int imass=0; imass<ana::nMass; imass++){
      clr.cd(imass+1);
      TF1 func("func", "[0]+[1]*fabs(x)+[2]*x*x", 0, 2.0);
      func.GetNpar();
      func.SetParameters(1, 1, 1);
      hlr[imass]->Fit(&func, "Q R", "", 0, 2.0);
      hlr[imass]->Fit(&func, "Q R", "", 0, 2.0);
      hlr[imass]->Fit(&func, "Q E R", "", 0, 2.0);
      auto fitResult = hlr[imass]->Fit(&func, "Q E R S", "", 0, 2.0);
      //fitResult->Print("v");
   ...
   for(int imass=0; imass<ana::nMass; imass++){
      ...
      int binlw =hlr[imass]->GetXaxis()->FindBin(0.);
   ...
   for(int iset=2; iset<3; iset++){
      ...
      for(int itrk=0; itrk<ntrk; itrk++){
         for(int ipt=0; ipt<nPt; ipt++){
            ...
            draw_proj1D_symm(dataset[iset], Form("../data/corr2D_trg_pd0_%s.root", dataMult[iset].c_str()), 
