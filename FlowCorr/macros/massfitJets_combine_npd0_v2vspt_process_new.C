...
void massfitJets_combine_npd0_v2vspt_process_new(
      const char* input_mc = "", 
      ...
      )
{
    ...
    for(int i=0;i<nPt;i++)
    {
        ...
        TGraphErrors* vn_data = (TGraphErrors*)file1->Get(Form("JetsvsMass_pt%d_dca%d",i,idca));
...
//        leg1->AddEntry(f3,"Combinatorial","l");
        ...
        c[i]->Print(Form("plots/dcaFull/v2/D0_mass_Jetsfit_combine_pt%d_y%.1f_dca%d.png",i, yMax, idca));
