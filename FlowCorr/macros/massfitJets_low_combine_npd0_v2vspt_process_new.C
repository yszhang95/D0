...
void massfitJets_low_combine_npd0_v2vspt_process_new(
      const char* input_mc = "", 
      ...
      )
{
    ...
    for(int i=0;i<nPt;i++)
    {
        ...
        TH1D* h_data = (TH1D*)file1->Get(Form("hMass_low_pt%d_dca%d", i, idca));
        ...
        tex->DrawLatex(0.22,0.86,"N_{trk}^{offline} < 35");
        ...
        TGraphErrors* vn_data = (TGraphErrors*)file1->Get(Form("Jets_lowvsMass_pt%d_dca%d",i,idca));
        ...
        ltx.DrawLatexNDC(0.65, 0.75, Form("Y_{jets}=%f", fvn_combinemassvnfit->GetParameter(13)));
        ltx.DrawLatexNDC(0.65, 0.7, Form("Y_{jets}Err=%f", fvn_combinemassvnfit->GetParError(13)));
...
//        leg1->AddEntry(f3,"Combinatorial","l");
    ...
    TGraphErrors* v2plot = new TGraphErrors(nPt, pt, v2, x_e,v2e);
    v2plot->Write(Form("Jetsplot_dca%d", idca));
    ofile.Close();
