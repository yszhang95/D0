...
void massfitVn_low_combine_pd0_ntrk_process(const char* input_mc = "",
      ...
      )
{
   ...
   const unsigned int n_trk_bin_ = 1;
   ...
   double sigfrac[n_trk_bin_];
    ...
    for(int i=0;i<n_trk_bin_;i++)
    {
        ...
        TH1D* h_data = (TH1D*)file1->Get("hMass_low");
        ...
        TGraphErrors* vn_data = (TGraphErrors*)file1->Get("V2_lowvsMass");
...
//        leg1->AddEntry(f3,"Combinatorial","l");
        ...
        c[i]->Print(Form("../plots/v2vsNtrk/D0_mass_Vn_lowfit_combine_trk%d_%s_%s.png",i, dataset.c_str(), str.c_str()));
