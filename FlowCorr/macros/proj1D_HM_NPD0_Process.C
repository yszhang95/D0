      ...
      )
{
   ...
   double yields_jet_ZYAM[ana::nMass][nPt][nDca];
   double yields_jet_ZYAM_err[ana::nMass][nPt][nDca];
   double yields_jet_low_ZYAM[ana::nMass][nPt][nDca];
   double yields_jet_low_ZYAM_err[ana::nMass][nPt][nDca];
   ...
   if(true){
      ...
      auto sr_pair = proj1D_shortrange_yields(h2DSignal_Ref_low, h2DBackground_Ref_low, 
            "sry_ref_low", c_sry_ref_low, 1, &h_sry_ref_low);
      auto lr_pair = proj1D_longrange_yields(h2DSignal_Ref_low, h2DBackground_Ref_low, 
            "lry_ref_low", c_lry_ref_low, 1, &h_lry_ref_low);
   ...
   TH1D* h_sry[nPt][nDca][ana::nMass];
   TH1D* h_lry[nPt][nDca][ana::nMass];
   for(int ipt=0; ipt<nPt; ipt++){
      for(int idca=0; idca<nDca; idca++){
      ...
      for(int idca=0; idca<nDca; idca++){
         for(int imass=0; imass<ana::nMass; imass++){
            ...
            auto sr_pair = proj1D_shortrange_yields(h2DSignal_D0[imass][ipt][idca], h2DBackground_D0[imass][ipt][idca], 
                  //dcacut_str.c_str(),
                  Form("sr_pt%d_dca%d_mass%d", ipt, idca, imass),
               c_sry[ipt][idca], imass+1, &h_sry[ipt][idca][imass]
               );

            auto lr_pair = proj1D_longrange_yields(h2DSignal_D0[imass][ipt][idca], h2DBackground_D0[imass][ipt][idca], 
                  //dcacut_str.c_str(),
                  Form("lr_pt%d_dca%d_mass%d", ipt, idca, imass),
               c_lry[ipt][idca], imass+1, &h_lry[ipt][idca][imass]
                  );
            auto yields_pair = calYields(sr_pair, lr_pair);
   ...
   TH1D* h_sry_low[nPt][nDca][ana::nMass];
   TH1D* h_lry_low[nPt][nDca][ana::nMass];
   for(int ipt=0; ipt<nPt; ipt++){
      for(int idca=0; idca<nDca; idca++){
      ...
      for(int idca=0; idca<nDca; idca++){
         for(int imass=0; imass<ana::nMass; imass++){
            ...
            auto sr_pair = proj1D_shortrange_yields(h2DSignal_D0_low[imass][ipt][idca], h2DBackground_D0_low[imass][ipt][idca], 
                  //dcacut_str.c_str(),
                  Form("sr_low_pt%d_dca%d_mass%d", ipt, idca, imass),
               c_sry_low[ipt][idca], imass+1, &h_sry_low[ipt][idca][imass]
               );
            auto lr_pair = proj1D_longrange_yields(h2DSignal_D0_low[imass][ipt][idca], h2DBackground_D0_low[imass][ipt][idca], 
                  //dcacut_str.c_str(),
                  Form("lr_low_pt%d_dca%d_mass%d", ipt, idca, imass),
               c_lry_low[ipt][idca], imass+1, &h_lry_low[ipt][idca][imass]
                  );
            auto yields_pair = calYields(sr_pair, lr_pair);
   ...
   TCanvas* csub[nPt][nDca];
   TCanvas* csub_low[nPt][nDca];
   for(int ipt=0; ipt<nPt; ipt++){
      for(int idca=0; idca<nDca; idca++){
         csub[ipt][idca] = new TCanvas(Form("csub_%d_%d", ipt, idca), "", 500*3, 400*5);
         csub[ipt][idca]->Divide(3, 5);
         TF1* f[ana::nMass];
         for(int imass=0; imass<ana::nMass; imass++){
            csub[ipt][idca]->cd(imass+1);
            h_sry[ipt][idca][imass]->Add(h_lry[ipt][idca][imass], -1);
            h_sry[ipt][idca][imass]->Draw("P");
            h_sry[ipt][idca][imass]->GetYaxis()->SetRangeUser(h_sry[ipt][idca][imass]->GetMinimum()-0.5, h_sry[ipt][idca][imass]->GetMaximum()*1.4);
            f[imass] = new TF1(Form("f_mass%d", imass), "[0]*x^2+[1]*x+[2]", 0.6, 2.2);
            f[imass]->SetParameters(1, 1, 1);
            h_sry[ipt][idca][imass]->Fit(f[imass], "REQ", "", 0.6, 2.2);
            h_sry[ipt][idca][imass]->Fit(f[imass], "REQ", "", 0.6, 2.2);
            h_sry[ipt][idca][imass]->Fit(f[imass], "REQ", "", 0.6, 2.2);
            double min = f[imass]->GetMinimum();
            double minX = f[imass]->GetMinimumX();
            TF1 minfunc("minfunc", "[0]", -0.5*TMath::Pi(), 1.5*TMath::Pi());
            minfunc.SetParameter(0, -min);
            h_sry[ipt][idca][imass]->Add(&minfunc);
            h_sry[ipt][idca][imass]->SetStats(0);
            double noZYAM_y_err;
            double noZYAM_y = h_sry[ipt][idca][imass]->IntegralAndError(h_sry[ipt][idca][imass]->FindBin(-1.2), h_sry[ipt][idca][imass]->FindBin(1.2), noZYAM_y_err, "width");
            double yields_err;
            //double yields = h_sry[ipt][idca][imass]->IntegralAndError(h_sry[ipt][idca][imass]->FindBin(0.), h_sry[ipt][idca][imass]->FindBin(1.5), yields_err, "width");
            //double yields = h_sry[ipt][idca][imass]->IntegralAndError(h_sry[ipt][idca][imass]->FindBin(0.0000001), h_sry[ipt][idca][imass]->FindBin(1.5), yields_err, "width");
            double yields = h_sry[ipt][idca][imass]->IntegralAndError(h_sry[ipt][idca][imass]->FindBin(-1.5), h_sry[ipt][idca][imass]->FindBin(1.5), yields_err, "width");
            //yields = 2*yields - h_sry[ipt][idca][imass]->GetBinContent(h_sry[ipt][idca][imass]->FindBin(0.)) * TMath::Pi()/16.;
            TLatex ltx;
            //ltx.DrawLatexNDC(0.45, 0.72, Form("Y_{w/oZYAM}=%g+/-%g", noZYAM_y, noZYAM_y_err));
            ltx.DrawLatexNDC(0.45, 0.65, Form("Y_{w/ZYAM}=%g+/-%g", yields, yields_err));
            //ltx.DrawLatexNDC(0.45, 0.58, Form("Y_{sr-lr}=%g+/-%g", yields_jet[imass][ipt][idca], yields_jet_err[imass][ipt][idca]));
            //yields_jet_ZYAM[imass][ipt][idca] = yields * 2.;
            //yields_jet_ZYAM_err[imass][ipt][idca] = yields_err*sqrt(2.);
            yields_jet_ZYAM[imass][ipt][idca] = yields ;
            yields_jet_ZYAM_err[imass][ipt][idca] = yields_err;
         }
         csub[ipt][idca]->Print(Form("test_pt%d_dca%d.png", ipt, idca));
         for(int imass=0; imass<ana::nMass; imass++){
            delete f[imass];
         }
         delete csub[ipt][idca];
      }
   }
   for(int ipt=0; ipt<nPt; ipt++){
      for(int idca=0; idca<nDca; idca++){
         csub_low[ipt][idca] = new TCanvas(Form("csub_low_%d_%d", ipt, idca), "", 500*3, 400*5);
         csub_low[ipt][idca]->Divide(3, 5);
         TF1* f[ana::nMass];
         for(int imass=0; imass<ana::nMass; imass++){
            csub_low[ipt][idca]->cd(imass+1);
            h_sry_low[ipt][idca][imass]->Add(h_lry_low[ipt][idca][imass], -1);
            h_sry_low[ipt][idca][imass]->Draw("P");
            h_sry_low[ipt][idca][imass]->GetYaxis()->SetRangeUser(h_sry_low[ipt][idca][imass]->GetMinimum()-0.5, h_sry_low[ipt][idca][imass]->GetMaximum()*1.4);
            f[imass] = new TF1(Form("f_mass%d", imass), "[0]*x^2+[1]*x+[2]", 0.6, 2.2);
            f[imass]->SetParameters(1, 1, 1);
            h_sry_low[ipt][idca][imass]->Fit(f[imass], "REQ", "", 0.6, 2.2);
            h_sry_low[ipt][idca][imass]->Fit(f[imass], "REQ", "", 0.6, 2.2);
            h_sry_low[ipt][idca][imass]->Fit(f[imass], "REQ", "", 0.6, 2.2);
            double min = f[imass]->GetMinimum();
            double minX = f[imass]->GetMinimumX();
            TF1 minfunc("minfunc", "[0]", -0.5*TMath::Pi(), 1.5*TMath::Pi());
            minfunc.SetParameter(0, -min);
            h_sry_low[ipt][idca][imass]->Add(&minfunc);
            h_sry_low[ipt][idca][imass]->SetStats(0);
            double noZYAM_y_err;
            double noZYAM_y = h_sry_low[ipt][idca][imass]->IntegralAndError(h_sry_low[ipt][idca][imass]->FindBin(-1.2), h_sry_low[ipt][idca][imass]->FindBin(1.2), noZYAM_y_err, "width");
            double yields_err;
            //double yields = h_sry_low[ipt][idca][imass]->IntegralAndError(h_sry_low[ipt][idca][imass]->FindBin(0.), h_sry_low[ipt][idca][imass]->FindBin(1.5), yields_err, "width");
            //double yields = h_sry_low[ipt][idca][imass]->IntegralAndError(h_sry_low[ipt][idca][imass]->FindBin(0.0000001), h_sry_low[ipt][idca][imass]->FindBin(1.5), yields_err, "width");
            double yields = h_sry_low[ipt][idca][imass]->IntegralAndError(h_sry_low[ipt][idca][imass]->FindBin(-1.5), h_sry_low[ipt][idca][imass]->FindBin(1.5), yields_err, "width");
            //yields = 2*yields - h_sry_low[ipt][idca][imass]->GetBinContent(h_sry_low[ipt][idca][imass]->FindBin(0.)) * TMath::Pi()/16.;
            TLatex ltx;
            //ltx.DrawLatexNDC(0.45, 0.72, Form("Y_{w/oZYAM}=%g+/-%g", noZYAM_y, noZYAM_y_err));
            ltx.DrawLatexNDC(0.45, 0.65, Form("Y_{w/ZYAM}=%g+/-%g", yields, yields_err));
            //ltx.DrawLatexNDC(0.45, 0.58, Form("Y_{sr-lr}=%g+/-%g", yields_jet_low[imass][ipt][idca], yields_jet_low_err[imass][ipt][idca]));
            //yields_jet_low_ZYAM[imass][ipt][idca] = yields *2;
            //yields_jet_low_ZYAM_err[imass][ipt][idca] = yields_err*sqrt(2);
            yields_jet_low_ZYAM[imass][ipt][idca] = yields;
            yields_jet_low_ZYAM_err[imass][ipt][idca] = yields_err;
         }
         csub_low[ipt][idca]->Print(Form("test_low_pt%d_dca%d.png", ipt, idca));
         for(int imass=0; imass<ana::nMass; imass++){
            delete f[imass];
         }
         delete csub_low[ipt][idca];
      }
   }
   ...
   TGraphErrors* g_Jets_ZYAM[nPt][nDca]; // signal + background, nMass points, need to do fitting, filled
   TGraphErrors* g_Jets_low_ZYAM[nPt][nDca]; // signal + background, nMass points, need to do fitting, filled
   ...
   for(int ipt=0; ipt<nPt; ipt++){
      for(int idca=0; idca<nDca; idca++){
         ...
         g_Jets_ZYAM[ipt][idca] = new TGraphErrors(ana::nMass);
         g_Jets_low_ZYAM[ipt][idca] = new TGraphErrors(ana::nMass);
      }
   }
   // trg ref
   ...
   for(int imass=0; imass<ana::nMass; imass++){
      for(int ipt=0; ipt<nPt; ipt++){
         for(int idca=0; idca<nDca; idca++){
            ...
            g_Jets_ZYAM[ipt][idca]->SetPoint(imass, hMass_D0_low[imass][ipt][idca]->GetMean(), yields_jet_ZYAM[imass][ipt][idca]);
            g_Jets_ZYAM[ipt][idca]->SetPointError(imass, 0, yields_jet_ZYAM_err[imass][ipt][idca]);

            g_Jets_low_ZYAM[ipt][idca]->SetPoint(imass, hMass_D0_low[imass][ipt][idca]->GetMean(), yields_jet_low_ZYAM[imass][ipt][idca]);
            g_Jets_low_ZYAM[ipt][idca]->SetPointError(imass, 0, yields_jet_low_ZYAM_err[imass][ipt][idca]);
         }
      }
   }
   // high N, ref
   ...
   for(int ipt=0; ipt<nPt; ipt++){
      for(int idca=0; idca<nDca; idca++){
         ...
         g_Jets_ZYAM[ipt][idca]->Write(Form("JetsZYAMvsMass_pt%d_dca%d", ipt, idca)); // signal + background, nMass points, need to do fitting, filled
         g_Jets_low_ZYAM[ipt][idca]->Write(Form("Jets_lowZYAMvsMass_pt%d_dca%d", ipt, idca)); // signal + background, nMass points, need to do fitting, filled
      }
   }
   ...
   for(int ipt=0; ipt<nPt; ipt++){
      for(int idca=0; idca<nDca; idca++){
         ...
         delete g_Jets_ZYAM[ipt][idca];
         delete g_Jets_low_ZYAM[ipt][idca];
      }
   }
...
std::pair<double, double> proj1D_shortrange_yields(TH2* h2DSignal, TH2* h2DBackground, const char* name, TCanvas* c, const int& ipad, TH1D** hsig)
{
   ...
   (*hsig) = h2DSignal->ProjectionY(Form("(*hsig)_%s", name), negBin, posBin);
   ...
   */
   ...
   auto h = (*hsig)->DrawCopy("E P", Form("sr_pad%d", ipad));
...
std::pair<double, double> proj1D_longrange_yields(TH2* h2DSignal, TH2* h2DBackground, const char* name, TCanvas* c, const int& ipad , TH1D** hsig)
{
   ...
   hNeg->Add(hPos);
   (*hsig) = new TH1D(*hNeg);
   (*hsig)->SetName(Form("(*hsig)_%s", name));
   delete hNeg;
   delete hPos;
   ...
   */
   ...
   (*hsig)->SetTitle(";#Delta#phi;dN/d(#Delta#phi)");
   (*hsig)->SetMarkerStyle(20);
   auto h = (*hsig)->DrawCopy("E P", Form("lr_pad%d", ipad));

   TLatex ltx;
   ltx.DrawLatexNDC(0.2, 0.8, "#font[42]{|#Delta#eta|>2}");
   ltx.DrawLatexNDC(0.58, 0.2, name);
