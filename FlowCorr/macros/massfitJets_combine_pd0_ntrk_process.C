      ...
      )
{
   ...
   double sigfrac[n_trk_bin_];
    ...
    //for(int i=0;i<1;i++)
    {
        ...
        TGraphErrors* vn_data = (TGraphErrors*)file1->Get(Form("JetsvsMass_trk%d",i));
...
//        leg1->AddEntry(f3,"Combinatorial","l");
        ...
        c[i]->Print(Form("../plots/v2vsNtrk/D0_mass_Jets_fit_combine_trk%d_%s_%s.png",i, dataset.c_str(), str.c_str()));
