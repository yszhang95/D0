...
void massfitJets_combine_pd0_ntrk_process(const char* input_mc = "",
      ...
      )
{
   ...
   double sigfrac[n_trk_bin_];
    ...
    //for(int i=0;i<1;i++)
    {
        ...
        TGraphErrors* jet_data = (TGraphErrors*)file1->Get(Form("JetsvsMass_trk%d",i));
...
//        leg1->AddEntry(f3,"Combinatorial","l");
        ...
        c[i]->Print(Form("../plots/v2vsNtrk/%s/D0_mass_Jets_fit_combine_trk%d_%s.png", dataset.c_str(), i, str.c_str()));
        c[i]->Print(Form("../plots/v2vsNtrk/%s/D0_mass_Jets_fit_combine_trk%d_%s.pdf", dataset.c_str(), i, str.c_str()));
