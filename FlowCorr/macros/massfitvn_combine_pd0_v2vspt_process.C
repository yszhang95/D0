...
void massfitvn_combine_pd0_v2vspt_process(const char* input_mc = "",
      ...
      )
{
   ...
   double sigfrac[nPt];
    ...
    //for(int i=0;i<1;i++)
    {
        TH1D* h_mc_match_signal = (TH1D*)file0->Get(Form("hMassPD0_pt%d", i));
        TH1D* h_mc_match_all = (TH1D*)file0->Get(Form("hMassPD0_All_pt%d", i));
        
        TH1D* h_data = (TH1D*)file1->Get(Form("hmass_pt%d",i));
        ...
        TH1D* h_Pt = (TH1D*)file1->Get(Form("hPt_pt%d",i));
        TH1D* h_KET = (TH1D*)file1->Get(Form("hKET_pt%d",i));
        pt[i] = h_Pt->GetMean();
        ...
        tex->DrawLatex(0.22,0.86,ntrk.c_str());
        ...
        TGraphErrors* vn_data = (TGraphErrors*)file1->Get(Form("g_v2_pt%d",i));
...
//        leg1->AddEntry(f3,"Combinatorial","l");
        ...
        float Chi2v2=0;
        int ndfv2 = 0;
        if(dataset=="PAHM1-6" || dataset=="PPHM") ndfv2=14 - 2;
        else ndfv2 = 13 - 2;
        ...
        c[i]->Print(Form("../plots/v2vspt/d0ana/D0_mass_vnfit_combine_v2vspt_%d_%s_%s.png",i, dataset.c_str(), str.c_str()));
